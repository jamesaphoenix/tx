/**
 * @fileoverview ESLint rule that prevents supervision SQL references outside core repositories
 *
 * This rule enforces the ownership boundary from DD-039: adapter layers (dashboard server,
 * CLI, scripts) must not embed direct SQL references to worker_sessions. All supervision
 * persistence must go through packages/core/src/repo/.
 *
...
const DEFAULT_TABLE_PATTERNS = ['worker_sessions'];
...
export default {
  meta: {
    type: 'problem',
    docs: {
      description: 'Disallow supervision SQL (worker_sessions) references outside core repositories',
      category: 'Best Practices',
      recommended: true
    },
    messages: {
      noSupervisionSql: 'Supervision SQL must be in packages/core/src/repo/, not adapter layers. Use core SupervisionService instead. Detected table: {{table}}'
    },
    schema: [
      {
        type: 'object',
        properties: {
          allowedPaths: {
            type: 'array',
            items: { type: 'string' },
            description: 'File path prefixes where supervision SQL is allowed'
          },
          tablePatterns: {
            type: 'array',
            items: { type: 'string' },
