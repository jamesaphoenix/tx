...
LEARNINGS=$(tx memory list -n 5 --json 2>/dev/null || echo "[]")
...
if [ -n "$TASK_ID" ]; then
  ...
  CONTEXT=$(tx memory context "$TASK_ID" --json 2>/dev/null || echo "")
...
SEARCH_RESULTS=$(tx memory search "$SEARCH_QUERY" -n 3 --json 2>/dev/null || echo "[]")
...
if [ -n "$SESSION_ID" ] && [ -n "$TRANSCRIPT_PATH" ] && [ -f "$TRANSCRIPT_PATH" ]; then
  ...
  tx memory list -n 10 --json > "$ARCHIVE_DIR/recent-learnings.json" 2>/dev/null || true
...
```bash
#!/bin/bash
# Install Claude Code hooks for tx memory integration
