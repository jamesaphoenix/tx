...
kind: spec
spec_type: prd
name: "PRD-010-contextual-learnings-system"
title: "PRD-010: Contextual Learnings System"
status: draft
version: 1
...
|----|-------------|-------------|
| CL-001 | Add learning manually | `tx memory add "content" [-c category]` |
| CL-002 | Search learnings | `tx memory search "query" [-n limit]` |
| CL-003 | List recent learnings | `tx memory list [-n limit]` |
| CL-004 | Record outcome feedback | `tx memory set <id> [--score 0.8]` |
| CL-005 | Import from CLAUDE.md | `tx memory index [file]` |
| CL-006 | Generate embeddings | `tx memory index` |
| CL-025 | View single learning by ID | `tx memory show <id> [--json]` |

### Task Context Retrieval

| ID | Requirement | CLI Command |
|----|-------------|-------------|
| CL-007 | Get learnings for task | `tx memory context <task-id>` |
| CL-008 | JSON output | `tx memory context <task-id> --json` |
| CL-009 | Inject to temp file | `tx memory context <task-id> --inject` |
| CL-010 | Token budget limit | `tx memory context <task-id> --max-tokens 2000` |
...
```bash
$ tx memory add "Always use transactions for multi-step DB operations" -c database
...
```bash
$ tx memory search "database transactions" -n 5
5 results:
...
- `tx memory index` prints: "No embedding model configured"
- Vector search falls back to BM25-only
- All other commands work

### Without Any Learnings

- `tx memory context <task-id>` returns empty list
- `tx memory search` returns empty list
