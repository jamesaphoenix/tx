...
│                             │                                           │
│                             └─► "tx memory add" if insights gained      │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| HC-008 | UserPromptSubmit calls `tx memory context <task-id>` if task mentioned |
...
| HC-011 | Stop blocks if `.tx/current-task` exists, prompts for memory capture |
...
```bash
#!/bin/bash
LEARNINGS=$(tx memory list -n 5 --json 2>/dev/null)
...
2. If yes, call `tx memory context <task-id> --json`
3. If no, call `tx memory search "<prompt keywords>" -n 3 --json`
...
3. If yes, block stop and prompt for `tx memory add`
...
{
  "decision": "block",
  "reason": "Please capture learnings with tx memory add before finishing."
}
```
...
- [PRD-010: Contextual Learnings System](./PRD-010-contextual-learnings-system.md) - Learning storage and retrieval
- [DD-011: Claude Code Hooks Implementation](../design/DD-011-claude-code-hooks.md) - Full script implementations
