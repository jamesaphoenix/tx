...
`tx auto reflect` aggregates data across recent agent sessions to produce a structured retrospective. It surfaces metrics on throughput, proliferation, stuck tasks, and actionable signals.
...
tx auto reflect --hours 1

# JSON output for scripting
tx auto reflect --json

# With LLM analysis
tx auto reflect --analyze

# Combine options
tx auto reflect --sessions 5 --hours 2 --analyze --json
```
...
**Tool name:** `tx_auto_reflect`

```json
{
  "name": "tx_auto_reflect",
  "arguments": {
    "sessions": 5,
    "hours": 2,
    "analyze": true
...
SIGNALS=$(tx auto reflect --hours 1 --json | jq -r '.signals[].type')

if echo "$SIGNALS" | grep -q "HIGH_PROLIFERATION"; then
  tx auto guard set --max-pending 20 --enforce
  tx sync compact
fi

if echo "$SIGNALS" | grep -q "STUCK_TASKS"; then
  STUCK=$(tx auto reflect --json | jq -r '.stuckTasks[].id')
  for id in $STUCK; do
...
- [`tx auto guard`](/docs/primitives/guard): Set task creation limits and tune them with reflect signals
...
- [`tx sync compact`](/docs/primitives/compact): Archive completed tasks
