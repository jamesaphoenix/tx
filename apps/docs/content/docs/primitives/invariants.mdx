...
- PRD `ears_requirements` and legacy `requirements`,
- design `goals`.

For legacy docs that use block-scalar strings for `requirements` or `goals`, sync normalizes non-empty lines into invariant candidates.
