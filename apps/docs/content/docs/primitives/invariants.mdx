...
Invariant management is accessed through the docs API endpoints and the CLI/MCP tools. The REST API provides doc-level operations that include invariant data.

```bash
# Get the doc graph (includes invariant counts)
curl http://localhost:3001/api/docs/graph

# Get a doc (invariant count included in metadata)
curl http://localhost:3001/api/docs/auth-design
```
