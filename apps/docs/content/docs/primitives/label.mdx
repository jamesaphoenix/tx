...
> **Note:** Label management (add, assign, unassign, list) is currently CLI-only. The SDK supports label-based filtering via `tx.tasks.ready()`.
...
```
GET /api/tasks/ready?labels=phase:discovery&limit=5
GET /api/tasks/ready?excludeLabels=needs-review
```

> **Note:** Label management (add, assign, unassign, list) is currently available via CLI only. The REST API supports label-based filtering on the ready and list endpoints.
