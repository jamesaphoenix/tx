---
title: tx pin
description: Pin key-value metadata to CLAUDE.md / AGENTS.md files
---

## Purpose

`tx pin` manages named content blocks ("pins") that sync project metadata into agent context files such as `CLAUDE.md` or `AGENTS.md`. Each pin is stored in the database and rendered as an XML-tagged block in target files:
...
- Run tx sync migrate status to verify"
...
- [`tx memory`](/docs/primitives/memory) - Search and manage knowledge
