...
Memory provides filesystem-backed document search over markdown directories. It indexes **your existing `.md` files** — project notes, design docs, meeting notes, learnings, and wikis — then makes them searchable via BM25 keyword search, vector similarity, and graph expansion through wikilinks and frontmatter relationships.
...
### Task Context & File Learnings

| Command | Description |
|---------|-------------|
| `tx memory context <task-id>` | Get task-relevant memory for prompt injection |
...
- [`tx pin`](/docs/primitives/pin) - Pin always-on metadata to `CLAUDE.md` or `AGENTS.md`
