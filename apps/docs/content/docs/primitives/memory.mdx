...
const source = await tx.memorySourceAdd('/Users/me/notes')
// Returns: { id, rootDir, label, createdAt }

// Register with a label
const docs = await tx.memorySourceAdd('./docs', 'Project Docs')
...
await tx.memorySourceRemove('/Users/me/old-notes')
```
...
const sources = await tx.memorySourceList()
...
const doc = await tx.memoryDocumentCreate({
...
const doc = await tx.memoryDocumentGet('mem-a1b2c3d4e5f6')
...
// List all documents
const docs = await tx.memoryDocumentList()

// Filter by source and tags
const filtered = await tx.memoryDocumentList({
...
// Basic keyword search
const results = await tx.memorySearch({ query: 'authentication flow' })

// Combined: semantic + graph expansion with filters
const expanded = await tx.memorySearch({
...
// Full re-index
const result = await tx.memoryIndex()
// Returns: { indexed, skipped, removed }

console.log(`Indexed: ${result.indexed}, Skipped: ${result.skipped}`)

// Incremental index (only changed files)
const incremental = await tx.memoryIndex({ incremental: true })

// Check indexing status
const status = await tx.memoryIndexStatus()
...
await tx.memoryTagAdd('mem-a1b2c3d4e5f6', ['backend', 'api', 'v2'])
```
...
await tx.memoryTagRemove('mem-a1b2c3d4e5f6', ['wip', 'experimental'])
```
...
await tx.memoryPropertySet('mem-a1b2c3d4e5f6', 'status', 'reviewed')
await tx.memoryPropertySet('mem-a1b2c3d4e5f6', 'owner', 'team-backend')
```
...
await tx.memoryPropertyRemove('mem-a1b2c3d4e5f6', 'status')
```
...
const props = await tx.memoryProperties('mem-a1b2c3d4e5f6')
...
await tx.memoryRelate('mem-a1b2c3d4e5f6', 'mem-f6e5d4c3b2a1')

// Or relate by filename
await tx.memoryRelate('mem-a1b2c3d4e5f6', 'login-flow.md')
```
...
const links = await tx.memoryLinks('mem-a1b2c3d4e5f6')
...
const backlinks = await tx.memoryBacklinks('mem-a1b2c3d4e5f6')
// Returns: Array<{ id, sourceDocId, targetDocId, targetRef, linkType, createdAt }>
...
await tx.memoryLinkCreate('mem-a1b2c3d4e5f6', 'mem-f6e5d4c3b2a1')

// Or link by filename
await tx.memoryLinkCreate('mem-a1b2c3d4e5f6', 'deployment-guide.md')
```
