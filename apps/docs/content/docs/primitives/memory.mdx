...
import { MemoryVsLearningsFigure } from '@/components/memory-figures'
...
<Mermaid
  ...
  caption="Register a markdown source once, index it into a searchable graph, then query that graph without moving your files into tx."
  chart={`flowchart LR
    Sources[markdown docs] --> Index[tx memory index]
    Index --> Graph[searchable graph]
    Graph --> Query[tx memory search]
...
<Mermaid
  title="Search Mode"
  caption="Keep the search stack simple: start with BM25, add semantic matching for fuzzy recall, then add graph expansion when related docs matter."
  chart={`flowchart LR
    Query[query] --> BM25[BM25 default]
    BM25 --> Semantic[optional --semantic]
    Semantic --> Expand[optional --expand]
    Expand --> Results[ranked results]
  `}
/>

<MemoryVsLearningsFigure />
## Commands
