...
// List runs via the REST API directly
const res = await fetch('http://localhost:3456/api/runs?limit=20&status=failed')
const { runs, hasMore, nextCursor } = await res.json()

// The SDK exposes heartbeat, stalled, and reap operations
...
// Get run details via REST API directly
const res = await fetch('http://localhost:3456/api/runs/run-abc12345')
const { run, messages } = await res.json()
...
```typescript
// Transcript messages are included in the run detail REST response
const res = await fetch('http://localhost:3456/api/runs/run-abc12345')
const { run, messages } = await res.json()
```
...
```typescript
// List failed runs via REST API
const res = await fetch('http://localhost:3456/api/runs?status=failed&limit=10')
