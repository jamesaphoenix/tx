...
// Claim a task with lease
const claim = await tx.claims.claim({
  taskId: 'tx-abc123',
  workerId: 'worker-1',
...
```bash
curl -X POST http://localhost:3456/api/claims \
  -H "Content-Type: application/json" \
  -d '{"taskId": "tx-abc123", "workerId": "worker-1", "leaseDurationMinutes": 60}'
```
...
```bash
DELETE /api/claims/:taskId/:workerId
```

### Example

```bash
curl -X DELETE http://localhost:3456/api/claims/tx-abc123/worker-1
```
...
```bash
POST /api/claims/:taskId/:workerId/renew
```

### Example

```bash
curl -X POST http://localhost:3456/api/claims/tx-abc123/worker-1/renew
```
