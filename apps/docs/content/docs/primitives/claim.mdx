...
<Mermaid
  ...
  caption="Claims stay active until they are renewed, released, or allowed to expire."
  chart={`sequenceDiagram
    ...
    Worker->>TX: tx claim task worker-1
    TX->>Store: create active claim + expiry
    Worker->>TX: tx claim renew task worker-1
    TX->>Store: extend expiry + renewal count
    Worker->>TX: tx claim release task worker-1
    Store-->>TX: task becomes claimable again
  `}
/>

<Mermaid
  title="Collision Block"
  caption="Claims close the race window after `tx ready` so only one worker keeps the task."
  chart={`sequenceDiagram
    autonumber
    ...
    W1->>TX: tx ready -> tx-abc123
    W2->>TX: tx ready -> tx-abc123
    W1->>TX: tx claim tx-abc123 worker-1
    TX-->>W1: lease granted
    W2->>TX: tx claim tx-abc123 worker-2
    TX-->>W2: already claimed
    W2->>TX: tx ready (next task)
  `}
/>
...
This is the recommended approach for multi-worker setups. See [`tx ready`](/docs/primitives/ready) for full details.
