...
- [`tx doc`](/docs/primitives/docs): structured PRDs and design docs on disk so the spec has a canonical source
...
`tx spec discover` refreshes doc-derived invariants automatically before scanning tests.
...
<Mermaid
  ...
  chart={`flowchart LR
    Docs["Docs"] --> Derived["Derived invariants"]
    Derived --> Map["Traceability mappings"]
