...
Decisions are first-class artifacts in tx. When agents or developers make implementation choices, those decisions are captured, reviewed, and optionally synced back to docs. This closes the spec-driven development loop: code changes produce decisions, which feed back to update docs and tests.
...
# Spec health (aggregates decisions + docs + tests)
tx spec health
tx spec health --json
```
...
Spec Health: DRIFTING
  ...
  Spec -> Test:  18/21 invariants have tests (86%)
  Spec State:    14 passing, 1 failing, 3 untested, 3 uncovered
  Doc Closure:   2 COMPLETE, 1 HARDEN, 1 BUILD
  Decisions:     3 pending, 1 approved-unsynced
  Doc Drift:     0 of 4 docs with hash mismatch
```

The `spec health` command aggregates decision status, doc drift, and spec/test coverage into a single health view.
Treat it as a portfolio rollup over the repo, not as a required step in the core `discover -> run -> fci/status -> complete` loop.
It also shows spec execution state and doc-level closure, so `synced` means the docs with active invariants are actually closed, not merely linked.
