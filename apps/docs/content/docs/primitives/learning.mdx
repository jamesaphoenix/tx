...
The CLI learning commands store learnings as tagged markdown documents via [`tx memory`](/docs/primitives/memory), while the SDK, MCP, and REST API use the dedicated learnings service backed by its own database tables. Both paths serve the same purpose — recording and retrieving knowledge — but use different storage backends.
...
```bash
tx learning helpful <id> [--score <0-1>] [--json]
```

**Examples:**

```bash
# Record a learning as helpful (default score 1.0)
tx learning helpful 42

# Record with a specific helpfulness score
tx learning helpful 42 --score 0.8

# JSON output
tx learning helpful 42 --json
```
...
| `score` | `number` | No | Helpfulness score between 0 and 1 (default: 1.0) |
...
```bash
# Use a specific local GGUF file or model URI
TX_LOCAL_EMBEDDING_MODEL_URI=/absolute/path/to/model.gguf

# Optional model cache/download directory
TX_LOCAL_EMBEDDING_MODEL_DIR=~/.node-llama-cpp/models

# Auto-download model URI if missing
TX_LOCAL_EMBEDDING_DOWNLOAD=1

# Expected embedding dimensions for the local model
TX_LOCAL_EMBEDDING_DIMENSIONS=768
```
...
| Arg | Type | Required | Description |
|-----|------|----------|-------------|
| `path` | `string` | No | File path to query learnings for |
