...
// Export to default path (.tx/tasks.jsonl)
const result = await tx.sync.export()
console.log(`Exported ${result.opCount} operations to ${result.path}`)

// Export to custom path
const custom = await tx.sync.export('./backup/tasks.jsonl')
```
...
// Import from default path (.tx/tasks.jsonl)
const result = await tx.sync.import()
console.log(`Imported: ${result.imported}, Skipped: ${result.skipped}`)

// Import from custom path
const custom = await tx.sync.import('./backup/tasks.jsonl')
```
...
console.log(`DB tasks: ${status.dbTaskCount}`)
console.log(`JSONL ops: ${status.jsonlOpCount}`)
...
Auto-sync is typically configured through the CLI. The current auto-sync state is available via the sync status:
...
// Check auto-sync state via status
const status = await tx.sync.status()
...
Auto-sync is typically configured through the CLI. The current auto-sync state is included in the `tx_sync_status` tool response:
...
// Compact JSONL file
const result = await tx.sync.compact()
console.log(`Before: ${result.before}, After: ${result.after}`)

// Compact a custom path
const custom = await tx.sync.compact('./backup/tasks.jsonl')
```
