---
title: tx msg send / tx msg inbox
...
<Mermaid
  title="Read Without Consume"
  caption="Inbox reads are pure. Multiple consumers can see the same pending message until someone explicitly acknowledges it."
  chart={`sequenceDiagram
    ...
    Planner->>TX: tx msg send team "review this"
    A->>TX: tx msg inbox team --after 0
    TX-->>A: pending message
    B->>TX: tx msg inbox team --after 0
    TX-->>B: same pending message
    A->>TX: tx msg ack 42
  `}
/>
...
<Mermaid
  title="Message State"
  caption="Only send, ack, TTL expiry, and garbage collection change message state. Reads just move the consumer cursor."
  chart={`stateDiagram-v2
    [*] --> Pending: tx msg send
    Pending --> Pending: tx msg inbox / cursor read
    Pending --> Acked: tx msg ack or msg ack all
    Pending --> Expired: TTL elapsed
    Acked --> GC: outbox GC
    Expired --> GC: outbox GC
    GC --> [*]
  `}
/>
...
| Parameter | Required | Description |
|-----------|----------|-------------|
| `ackedOlderThanHours` | No | Remove acked messages older than N hours (positive integer) |
