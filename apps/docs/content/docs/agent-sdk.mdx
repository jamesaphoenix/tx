...
<Callout type="info" title="HTTP Actor Header">
...
<Callout type="info" title="Unified Memory Primitive">
The `tx.learnings.*` and `tx.context.*` SDK methods map to the unified [`tx memory`](/docs/primitives/memory) primitive. The SDK API names are unchanged for backwards compatibility, but the underlying functionality is part of the memory system. Use `tx memory add` with `-t learning` tags at the CLI level for the equivalent workflow.
</Callout>
...
#### `tx.decompose.run(data)`

Preview or materialize an explicit task graph from a design doc.

Use `dryRun: true` to validate the graph before creating tasks. The response
includes the root preview, the validated plan, any materialized tasks, and the
linked root task when writes occur.
...
const graph = await tx.decompose.run({
  docRef: "auth-flow-design",
...
- [tx doc](/docs/primitives/docs) - Docs-first authoring
- [tx spec](/docs/primitives/spec-trace) - Spec discovery, scoring, and closure
- [tx memory](/docs/primitives/memory) - Memory and contextual retrieval
- [tx label](/docs/primitives/label) - Ready queue scoping by phase
- [tx guard](/docs/primitives/guard) - Task creation limits (bounded autonomy)
...
- [tx msg send / tx msg inbox](/docs/primitives/inbox) - Agent-to-agent messaging
- [tx sync](/docs/primitives/sync) - Stream-based event sync
- [tx trace](/docs/primitives/traces) - Run and failure observability
