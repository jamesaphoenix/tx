...
1. **Task Management**: stay with [Primitives](/docs/primitives) and focus on `ready`, `done`, `block`, `show`, and `sync`.
...
4. **Bounded Autonomy**: add [`tx label`](/docs/primitives/label), [`tx guard`](/docs/primitives/guard), [`tx verify`](/docs/primitives/verify), [`tx reflect`](/docs/primitives/reflect), and [`tx gate`](/docs/primitives/gate).
5. **Coordination**: add [`tx claim`](/docs/primitives/claim), channel messaging via [`tx inbox`](/docs/primitives/inbox), and shared group context when multiple actors are involved.
