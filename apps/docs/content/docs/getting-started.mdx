...
<Mermaid
  title="Work Loop"
  caption="Pull one ready task, inspect it, implement, verify, then either finish it or keep iterating."
  chart={`flowchart TB
    Ready[tx ready] --> Found{task found?}
    Found -->|No| Exit[stop]
    Found -->|Yes| Read[show + context]
    Read --> Work[implement]
    Work --> Pass{verify passes?}
    Pass -->|Yes| Done[tx done]
    Pass -->|No| Iterate[fix or block]
  `}
/>
...
2. **[Bounded Autonomy](/docs/primitives/gate)** -- prevent runaway agents with `gate`, `guard`, `verify`, `label`, and `reflect`.
