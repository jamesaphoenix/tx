...
This page is the final, advanced stage of the tx docs journey. Use it after you are already comfortable with [Primitives](/docs/primitives) and the [Headful Experience](/docs/headful).

Use watchdog when you need detached, long-running RALPH loops with automatic health checks and stale-run recovery.

For the manual non-detached workflow itself, see [Ralph Loop](/docs/ralph-loop).

## When to Enable Watchdog

Enable watchdog only when all of the following are true:

- Your normal loop (`tx ready`, `tx done`, `tx dep block`) is stable.
...
1. Follow the [Rollback](#rollback) section.
