...
import type { TestDatabase } from "../src/database/index.js"
import {
  createTestDatabase,
  TestDatabaseService,
  TestDatabaseLive
...
describe("createTestDatabase", () => {
  ...
  it("should create an in-memory database with migrations applied", () => {
    ...
    const result = db.query<{ version: number }>("SELECT MAX(version) as version FROM schema_version")
  ...
  it("should allow inserting and querying data", () => {
    ...
    const tasks = db.query<{ id: string; title: string }>("SELECT id, title FROM tasks")
  ...
  it("should support query with parameters", () => {
    ...
    const tasks = db.query<{ id: string }>("SELECT id FROM tasks WHERE title = ?", ["Test Task"])
    expect(tasks).toHaveLength(1)
    expect(tasks[0].id).toBe("tx-abc12345")
  })
  ...
  it("should reset database by clearing all user tables", async () => {
    ...
    const schemaVersion = db.query<{ version: number }>("SELECT MAX(version) as version FROM schema_version")
...
describe("TestDatabaseLive Layer", () => {
  it("should provide TestDatabaseService through Layer", async () => {
    const program = Effect.gen(function* () {
      ...
      return testDb.query<{ version: number }>("SELECT MAX(version) as version FROM schema_version")
    })
